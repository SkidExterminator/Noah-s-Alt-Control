--[[                                                              
`7MM"""Yp, `YMM'   `MM'    `7MN.   `7MF'  .g8""8q.         db      `7MMF'  `7MMF'
  MM    Yb   VMA   ,V        MMN.    M  .dP'    `YM.      ;MM:       MM      MM  
  MM    dP    VMA ,V         M YMb   M  dM'      `MM     ,V^MM.      MM      MM  
  MM"""bg.     VMMP          M  `MN. M  MM        MM    ,M  `MM      MMmmmmmmMM  
  MM    `Y      MM           M   `MM.M  MM.      ,MP    AbmmmqMA     MM      MM  
  MM    ,9      MM           M     YMM  `Mb.    ,dP'   A'     VML    MM      MM  
.JMMmmmd9     .JMML.       .JML.    YM    `"bmmd"'   .AMA.   .AMMA..JMML.  .JMML.                                                                              
]]
...
if LocalPLR.Name ~= Username then
    ...
    chat("Noah's Alt Control Running!")
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Thank You",
        Text = "Thank you for using Noah's Alt Control!",
        Time = 6
    })
    ...
    function commands(player, message)
        ...
        if msg == Prefix .. "credits" then
            ...
            if index == 1 then
                chat("This is a open source alt control script made by Noah.")
            end
        ...
        if msg:sub(1, 9) == Prefix .. "3drender" then
            ...
            function runCode()
                if switch == "disable" then
                    ...
                    showDefaultGui(true, "Noah's Alt Control Running (3drender disabled)")
        ...
        if msg:sub(1, 12) == Prefix .. "privacymode" then
            ...
            function runCode()
                ...
                elseif switch == "enable" then
                    showDefaultGui(true, "Noah's ALt Control Running (privacy mode enabled)")
